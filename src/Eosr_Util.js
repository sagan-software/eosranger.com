...
function decodeResult(decoder, json) {
  var exit = 0;
  var decoded;
  try {
...
function decodeOption(decoder, json) {
  var match = decodeResult(decoder, json);
  if (match.tag) {
    return undefined;
  } else {
...
function decodeText(decoder, text) {
  return Js_option.andThen((function (json) {
                return decodeOption(decoder, json);
              }), Json.parse(text));
}
